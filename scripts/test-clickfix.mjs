...
const brick=(step,session='S')=>page.locator(`[data-cache-bricks-brick="${session}:1:${step}:0"][data-cache-bricks-face="cache"]`)
const panel=()=>page.locator('[data-cache-bricks-panel]')
const ready=()=>page.locator('[data-cache-bricks-transcript-state="ready"]').waitFor()
const landed=()=>page.locator('[data-cache-bricks-landed]')
...
check('Single click automatically opens conversation, not statistics',await panel().locator('[data-cache-bricks-transcript="assistant"]').innerText()==='S ANSWER A')
...
check('Landing has visible accent',await page.evaluate(()=>getComputedStyle(document.querySelector('[data-cache-bricks-landed]')).boxShadow)!=='none')
...
check('Enter opens matching preview',await panel().locator('[data-cache-bricks-transcript="assistant"]').innerText()==='S ANSWER B')
...
check('Late history read cannot replace newer preview',await panel().locator('[data-cache-bricks-transcript="assistant"]').innerText()==='S ANSWER B')
...
await brick(1).dblclick({delay:40});await page.locator('[data-cache-bricks-notice="error"]').waitFor({timeout:7000})
check('Missing target never highlights adjacent step',await landed().count()===0)
check('Missing target reports a visible failure',(await page.locator('[data-cache-bricks-notice="error"]').innerText()).includes('未找到'))
...
check('Session switch binds preview to new session',await panel().locator('[data-cache-bricks-transcript="assistant"]').innerText()==='T ANSWER B')
