...
//#endregion
//#region src/core/brick-ledger.ts
/**
	...
	*/
	startAttempt(observation) {
		...
		const draft = {
			...
			...pending === void 0 ? {} : { options: pending.options },
...
*/
function createCacheBricksRouter(deps, options = {}) {
	...
	*/
	...
	const collect = (sessionId, observation) => {
		...
		if (!state.warnedUnattributed && state.ledger.unattributedCount > 0) {
			...
			console.warn(`[dsh-cache-bricks] dropped ${String(state.ledger.unattributedCount)} observation(s) for session ${sessionId}: no attempt could be identified for them (see feed.unattributed).`);
		}
		if (state.listeners.size === 0) return;
