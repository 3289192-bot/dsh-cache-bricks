...
| 半边 | 内容 | 生效方式 |
|---|---|---|
| 宿主（Node） | 只读采集器 + 内容寻址存储 + `/cache-badge` 路由与 SSE | **必须重启 `dsh web`**（`link:` 安装省掉重装，但换 JS 模块要新进程） |
| 客户端（浏览器） | 砖块棋盘 + 7 Tab 面板 + Diff | HMR 轮询产物，重建后自动热替换；必要时刷新页面 |

**降级路径**：没有宿主半边（或旧版本）时，`/cache-badge/*` 返回 404，客户端**回退到自己的 per-step 折叠**，
砖块照常显示（面板没有记录可开）。采集器是增强，不是依赖。

**路由鉴权**：`/cache-badge/*` 先走 **Harness 自己的请求策略**（`ctx.connection.requestRejection`，
...
不凭一次"没有"给宿主定罪；要确认就打补丁后自查：`node patch.mjs --check`。取证、复现与最小宿主补丁
（含 5 条回归测试与真机验证）在 `host-patches/system-message-never-withdraw/`：补丁打在**宿主**
（`dsh-client-ui-chat` 的编译产物）上，**不进这个插件包**——第三方 Definition 无权替换内建的，
宿主的问题归宿主。
...
```sh
# 017（0.1.7-rc.2；rc.1 同样支持）
dsh plugin --profile web add 'link:<PLUGIN_PATH>'

# daily（0.1.6-alpha.1，全局安装的 dsh）
${env:DSH_HOME} = '<DSH_HOME_DAILY>'
dsh plugin --profile web add 'link:<PLUGIN_PATH>'
```

安装只写 profile 的 `dependencies`、把包名追加到 `dsh.profile.bundles`、建立链接；`cordis.patch.yml` 不动。
运行时**不会替你构建**，改完源码先 `pnpm run build`。
...
pnpm run test             # vitest：19 个文件，含构建产物测试
...
6. `panel-live` —— **对着运行中的实例闭环**：从 HTTP 取一块真实砖，用真实组件渲染 7 个 Tab 并核对
...
7. `scripts/ui-verify.mjs` —— **真浏览器**：棋盘是 React 之外的 DOM 覆盖层、两面是 CSS 3D 翻面、
   "点砖跳转"要动真实布局，这三件事只有真浏览器能证明（19 项检查）。
...
- **能否落到"还没加载的旧轮"取决于宿主**：0.1.7-rc.1 / rc.2 的 `system-message` Definition 会在 prepend 重建时
  ...
  插件侧不做任何 monkey-patch，只如实报告 `host-projection-blocked`；宿主打上
  `host-patches/system-message-never-withdraw/patch.mjs` 之后，实测**加载+落点 0.5 秒、exact**
...
**砖的正反两面（一次撤回的记录 + 一次改向）**：正面只有**一层**视觉——填充=缓存命中率（绿→黄→红，
...
| 降级路径（无宿主半边，按 step 折叠） | `none`（`client-fold`） | **不跳**：没有 attempt 身份，不声称任何行 |
...
| accuracy | 含义 | UI |
|---|---|---|
| `exact` | 落在**这块砖自己**的行上（上表四种之一） | 滚动 + 淡蓝高亮 + 行上打 `data-cache-badge-landed="<row>"` |
