...
| The **page pager** is `ISession.loadOlder(): Promise<void>` — "extend history by at least 50 messages and two Turn starts, including a partial Turn at the window's beginning. Stop at 500 messages or history exhaustion even when those minima cannot be met. Publish one prepend" (`PAGE_MESSAGES = 50`, `HISTORY_PAGE_OPTIONS = { maxMessages: 500, turnWindow: { minMessages: 50, minTurns: 2 } }`) | `dsh-api-session-controller/lib/types/client/contract/session.d.ts:121-127`, `.../client/sessions/session.js:19-26, 312-336` |
| `loadOlder()` is **fire-and-forget**: every path resolves `undefined`, it never rejects (`catch` swallows, `console.error` for anything but a remote failure), and its guard — `openState !== 'open' \|\| !hasMore \|\| loadingOlder` — makes a second call a **silent no-op** (no promise coalescing, unlike `open()` and `loadThrough`) | `.../client/sessions/session.js:312-336` |
| A page is prepended into the **same** window (`prependWindow` → `eventSource.prepend` → `window = concat(leaf(entries), window)`), and `hasMore` / `baseSeq` move with it — there is no second array and no window replacement | `.../client/sessions/session.js:650-661`, `.../client/contract/events.js:71-74` |
| **What "it worked" looks like:** only the window. `SessionSnapshot.hasMore` / `.loadingOlder` (`openState`/`openError` beside them) and `SessionEventWindow.hasMore` / `.revision` / `.change` — the oldest loaded seq is **not** exposed and must be derived from `entries[0].event.seq`; `hasOlder`, `oldestSeq` and `firstSeq` do not exist on the snapshot | `.../client/contract/snapshot.d.ts:58-80`, `.../client/contract/events.d.ts:41-61` |
| There is **no `history/prepend` session event** in this version. The two official signals are `eventSource.subscribe(listener)` (synchronous on every accepted mutation, `change.kind === 'prepend'` / `revision + 1`) and `session.subscribe(listener)` (a uSES subscription over `SessionSnapshot`, fired around a page by `notifier.markDirty()`), and both are **optional** — a core that publishes neither is read by polling | `.../client/contract/events.d.ts:63-70`, `.../client/sessions/session.js:320-334`, `KNOWN_SESSION_EVENT_TYPES` (59 names, no prepend) |
| A **step is bracketed** in the durable log: `step/start { turn, step }` … `assistant/message`/`assistant/attempt` (with the compact `stream` and `usage`) … `tool/call` / `tool/result` / `llm/retry` / `llm/retry-started` … `step/end { turn, step }`, and every one of those events carries `turn`/`step` in `data`. Measured on a real log: a retried step is `assistant/attempt` → `llm/retry` → `llm/retry-started` → `assistant/attempt` → … → `assistant/message`, all **inside one bracket** | `dsh-session/lib/types/types.d.ts:262-405`, measured: `sessions/--C-dshworkspace-0.1.7rc1--/session-24f42588-…` seq 712-721 |
| `request/header` and `request/context` are **sticky and rare**: the ledger applies the last one seen to every attempt after it, and the log writes them once per request series (one of each in a real 300-event window) — while `request/header`'s data carries **no** `turn`/`step` | `src/core/brick-ledger.ts:360-365`, `dsh-session/lib/types/types.d.ts:393-405`, measured: 1 header + 1 context in 300 events |
