 ...
 */
export function toneOf(usage: CacheUsage | undefined): BrickTone {
  const ratio = hitRatioOf(usage)
  if (ratio === null || usage === undefined) return 'unknown'
  if (promptTokensOf(usage) < MIN_SIGNIFICANT_TOKENS) return 'unknown'
  if (ratio < BAD_BELOW) return 'bad'
  return ratio < WARN_BELOW ? 'warn' : 'good'
}
...
export function brickLabel(brick: Pick<CacheBrick, 'hitRatio'>): string {
  return brick.hitRatio === null ? 'n/a' : percentLabel(brick.hitRatio)
}

/** Built from the settlement, so it needs no id counter and stays stable across a reload. */
export function brickIdOf(sessionId: string, turn: number, step: number, attempt: number): string {
