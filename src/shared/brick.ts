...
export function brickHeadline(record: BrickRecord): string {
  const where = `turn ${String(record.identity.turn)} · step ${String(record.identity.step)}`
  const attempt = record.identity.attemptOrdinal > 0 ? ` · retry ${String(record.identity.attemptOrdinal)}` : ''
  const ratio = record.metrics.cacheHitRatio
  const cache = ratio === undefined ? 'n/a' : `${(ratio * 100).toFixed(1)}%`
  const error = record.finish?.reason === 'error' ? ' · error' : ''
  const abort = record.interrupted === true ? ' · interrupted' : ''
  return `${where}${attempt} · ${cache}${error}${abort}`
}
