...
> ## 0.1.4 · GitHub 发布版
>
> 在 **0.1.3** 之上改的是**数据层的形状**：历史不再是一次读完的积压，而是**按场景加载**——
> 屏幕上有什么，就只把那一块日志重新读成砖；读者往左走到头，再按官方分页 `loadOlder()` 一页页把
> 更早的历史要进来。
>
> 这修掉的是 0.1.3 的一个真问题：那时回放整个已加载窗口，而账本固定只留 400 块砖，多出来的部分
> 由客户端 fold 补——**缓存读数还在，类型却退化成 `output`**（fold 看不到 reasoning/tool 通道，
> 它自己也是这么说的）。于是"离最新端越远、越是你特意走过去看的那一段，越没有类型"。现在场景之外
> 不重放、场景之内必定精确。
>
> 同时修掉一个更隐蔽的错：平移保持原来按"新增了多少列"计数，分不清**追加**与**前插**，于是每落一页
> 历史都会把正在看历史的读者往过去推 N 格。现在按"比上次见到的最新轮次更新了几列"计。
>
> 砖契约、数据模型、渲染规则与绝对配色（< 90% 黄、< 70% 红）都没有改动。设计记录见
> [docs/history-scene.md](docs/history-scene.md)。
>
> 0.1.3 修的是**定位**：命中 DOM 不算成功，看得见才算（长轮次的过程组自带滚动区，先滚内层、
> 重新测量、再滚外层；仍不可见就报 `exact-not-visible`，不高亮也不算成功）。
>
...
**走到最左边会自己续上更早的历史**（0.1.4）：棋盘只把**你看得见的那一块**读成精确的砖，离左边缘还剩
一屏时就去按官方 `loadOlder()` 要一页（50 条消息 / 2 个轮次起，最多 500 条）；页面落下时画面**不动**，
只是左边多出更多可以走过去的历史。要不到（`hasMore` 为假）就停，不会反复请求。
...
| `exact-not-visible` | **找到了它自己那一行，但行被裁掉了**：长轮次的过程组自带滚动区（`[data-step-process-body]`，官方 `max-height` 封顶），只滚外层对话时行会留在组内看不见。0.1.3 起定位**先滚内层、重新测量、再滚外层**；万一还是不可见，就报这一种、**不高亮、也不算定位成功** |
...
**命中 DOM ≠ 看得见。**（0.1.3）`exact` 的定义从"声明的行在 DOM 里"收紧成"**那行在读者能看到的框里**"：它必须同时落在内层过程组与外层对话的可见范围内，否则报 `exact-not-visible`。这条是被一个假成功逼出来的——高光确实打在了正确的 DOM 上，只是被过程组裁掉了，而面板还在说"已定位"。
