...
> ## 0.1.3 · GitHub 发布版
>
> 在 **0.1.2** 之上只修一件事：**定位：命中 DOM 不算成功，看得见才算**。长轮次的过程组自带滚动区
> （`[data-step-process-body]`，官方 `max-height` 封顶），此前只滚外层对话，于是行被组内裁掉——
> 高光确实打在正确的 DOM 上，屏幕却什么也看不到，面板还报"已定位"。现在**先滚内层、重新测量、
> 再滚外层**；仍然不可见时报 `exact-not-visible`，**不高亮、也不算定位成功**。
>
> 0.1.2 是公开版本线的起点（基于本地冻结的 0.1.1 稳定版与分发调整）；砖契约、数据模型、渲染规则与
> 绝对配色（< 90% 黄、< 70% 红）在这一版都没有改动。
>
